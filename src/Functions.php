...
if (!function_exists('get_public_or_private_cert')) {
     ...
     *
     * @return resource|string|\OpenSSLAsymmetricKey
     */
...
if (!function_exists('verify_wechat_sign')) {
    function verify_wechat_sign(ContainerInterface $container, MessageInterface $message, array $params): void
    {
        ...
        $public = get_public_or_private_cert(
            empty($public) ? reload_wechat_public_certs($container, $params, $wechatSerial) : $public,
            true
        );
...
if (!function_exists('reload_wechat_public_certs')) {
    function reload_wechat_public_certs(ContainerInterface $container, array $params, ?string $serialNo = null): string
    {
        $data = $container->get(Pay::class)->wechat()->pay(
        ...
        $container->get(ConfigInterface::class)->set('pay.wechat', [$params['_config'] ?? 'default' => $wechatConfig->all()]);
