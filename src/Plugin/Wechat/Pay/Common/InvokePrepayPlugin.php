...
class InvokePrepayPlugin implements PluginInterface
{
    ...
    public function assembly(Rocket $rocket, Closure $next): Rocket
    {
        ...
        $this->logger->info('[wechat][InvokePrepayPlugin] 插件开始装载', ['rocket' => $rocket]);

        $prepayId = $rocket->getDestination()->get('prepay_id');

        if (is_null($prepayId)) {
            $this->logger->error('[wechat][InvokePrepayPlugin] 预下单失败：响应缺少 prepay_id 参数，请自行检查参数是否符合微信要求', $rocket->getDestination()->all());
        ...
        $this->logger->info('[wechat][InvokePrepayPlugin] 插件装载完毕', ['rocket' => $rocket]);
